"""
Basic example: Fetch historical data for AAPL and load to Parquet files.
...
Data is stored in Parquet format partitioned by date and symbol.
DuckDB is used in-memory to query the Parquet files.
...
def main():
    ...
    # Create pipeline to filesystem (Parquet)
    pipeline = dlt.pipeline(
        pipeline_name="ib_basic",
        destination=dlt.destinations.filesystem(bucket_url="data"),
        dataset_name="stocks",
    )
    logger.info(f"Pipeline created: {pipeline.pipeline_name} -> Parquet files in ./data/")
    ...
    logger.info(f"Parquet files location: {data_path}/")

    # Connect to in-memory DuckDB and query Parquet files
    conn = duckdb.connect(":memory:")

    # Register Parquet dataset
    conn.execute(f"""
        CREATE VIEW historical_bars AS
        SELECT * FROM parquet_scan('data/historical_bars/**/*.parquet', hive_partitioning=true)
    """)
    ...
    logger.info(f"Total rows in Parquet files: {result[0]}")
    ...
    if result[0] > 0:
        ...
        print("\n" + "="*80)
        print("📈 Latest 5 bars from Parquet files:")
        print("="*80)
...
def main_simple():
    ...
    info = pipeline.run(data, loader_file_format="parquet")
    print(f"✓ Pipeline finished! Data in: ./data/{pipeline.dataset_name}/")
